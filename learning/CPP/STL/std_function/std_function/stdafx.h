...
public:
	...
	struct CallableUnit {
		params_set _params;
		SunnyTcsAnyData _obj;
		SunnyTcsAnyData _func;
		invoke_func_ptr _invoke;
	...
	void _work(std::false_type) {
		if (_waitQueue.size() > 0) { //存在未执行任务
			CallableUnit& first = _waitQueue.front();
			((_Res*)_resultBuffer) ->~_Res();
			memset(_resultBuffer, 0, callable_storage_size);
			new(_resultBuffer) _Res( first._invoke(&first._params, &first._func, &first._obj)); //invoke执行
			first._manager(first._func, first._func, SunnyTcsFunctionBase::destroy_func);//对可调用实体的释放
			_waitQueue.pop();
		}
	}
