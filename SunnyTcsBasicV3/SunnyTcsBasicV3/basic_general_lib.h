...
class SunnyTcsNextResDefaultPolicy {
public:
	static _Ty getNextRes(const _Ty& val);

	static bool isLeftCanStepToRight(_Ty const&, _Ty const&);

	bool operator()(_Ty const&, _Ty const&);
...
//资源拥有者声明
template<typename _Ty>
class SunnyTcsUniqueIdKeeper;


//某种唯一资源的分配器
template<
	typename _Ty,
	template<typename T> class _NextResPolicy = SunnyTcsNextResDefaultPolicy,  //资源的渐进策略，比如ID资源为1，下一个资源是+1 ,或者-1
	template<typename, typename, typename   > class _Container//存储资源的优先队列, 你可以使用自实现版本
>
class SunnyTcsUniqueIdManager {
public:
	friend class SunnyTcsUniqueIdKeeper<_Ty>; //友元类，只有该类才能调用内部的资源获取与申请，归还函数，保证数据的安全

	typedef _NextResPolicy<_Ty> NextResPolicy;
	typedef _Container<_Ty, std::vector<_Ty>, NextResPolicy > Container;

	//def ctor
	explicit SunnyTcsUniqueIdManager(_Ty const&);
	//copy ctor
	SunnyTcsUniqueIdManager(const SunnyTcsUniqueIdManager&) = delete;
	//detor
	virtual ~SunnyTcsUniqueIdManager() {}
	//operator=
	SunnyTcsUniqueIdManager& operator=(const SunnyTcsUniqueIdManager&) = delete;
...
class SunnyTcsUniqueIdKeeper {
public:
	typedef SunnyTcsUniqueIdManager<_Ty> IdManager;

	//ctor
	explicit SunnyTcsUniqueIdKeeper(IdManager* idm);
	//copy ctor
	SunnyTcsUniqueIdKeeper(const SunnyTcsUniqueIdKeeper&) = delete;

	//detor
	virtual ~SunnyTcsUniqueIdKeeper() {
		_idm->returnId(_res);
	}

	SunnyTcsUniqueIdKeeper& operator= (const SunnyTcsUniqueIdKeeper&) = delete;

	//申请更换资源
	bool changeRes(_Ty newId) {
		if (_idm->applyForId(newId)) {
			_idm->returnId(_res);
			_res = newId;
			return true;
		}
	...
	inline _Ty getRes()const { return _res; }
...
namespace basic {
	...
	//std::_if的自实现版本
	template<bool val, typename _Ty1, typename _Ty2>
	struct SunnyTcsIf {
		typedef _Ty2 type;
	};
	...
	struct SunnyTcsIf<true, _Ty1, _Ty2> {
		typedef _Ty1 type;
	};
	...
	*/
	...
	private:
		...
		typedef typename SunnyTcsIf< arg_count == 0, SunnyTcsVoid, std::tuple<_Args...>>::type params_set;
	...
	public:
		...
		typedef typename SunnyTcsIf<std::is_same<void, result_type>::value, std::true_type, std::false_type>::type isResultVoid;
