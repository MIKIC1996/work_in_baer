...
SunnyTcsGraphicsVehicle::SunnyTcsGraphicsVehicle(const SunnyTcsGraphicsCoorSys * cs, const SunnyTcsMapAdjuster * ad,QPointF pt)
	: SunnyTcsMapGraphicItem("agv"),_cs(cs),_ad(ad), _ladarRadius(1000)
{
	this->setPos(pt);
	this->setZValue(5);
}
...
{
	//默认方法
	painter->fillRect(QRectF(-1000,-1000,2000,2000),Qt::gray);
}


QPainterPath SunnyTcsGraphicsVehicle::shape() const
{
	QPainterPath path;
	path.addEllipse(QPointF(0, 0), _ladarRadius, _ladarRadius);
	return path;
}
