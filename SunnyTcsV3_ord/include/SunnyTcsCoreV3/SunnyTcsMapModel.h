...
namespace core {
	...
	public:
		A_SunnyTcsModelData(SunnyTcsMapCoorSys* cs = nullptr,
			...
			:  _cs(cs), _modelId(id), _len(len), _wid(wid), _resolution(resolution),
			_ves(), _pts(), _paths()
		{}
		...
		inline qint32 getLen()const { return _len; }
		inline qint32 getWid()const { return _wid; }
		inline qint32 getResolution()const { return _resolution; }

		//setter
		void setLen(qint32 len) { _len = len; }
		void setWid(qint32 wid) { _wid = wid; }
